...
// if(!$user){
//     header('Location: http://localhost\dashboard\includes\login\login.php');
// }
...
<head>
    ...
    <title>Login - Nuevo Password</title>
                                    ...
                                    <form class="user" method="POST">
                                        ...
                                        <div class="form-group">
                                            <input type="password" class="form-control form-control-user"
                                                id="" name="passwordNuevo" placeholder="Nuevo Password">
                                            <input type="password" class="form-control form-control-user"
                                                id="" name="passwordNuevoConfirmar" placeholder="Confirmar Password">
                                        </div>
                                        
                                        <input type="submit" name = "nuevoPass" class="btn btn-primary btn-user btn-block">
...
if($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    $passwordNuevo = mysqli_real_escape_string($db, $_POST['passwordNuevo']);
    $passwordNuevoConfirmar = mysqli_real_escape_string($db, $_POST['passwordNuevoConfirmar']);
    $error = false;
    ...
    if ($passwordNuevoConfirmar === "") {
        echo '<script>swal("Atención", "El Password es obligatorio", "error");</script>';
        $error = true;
    }

    if ($passwordNuevo != $passwordNuevoConfirmar) {
        echo '<script>swal("Atención", "Los Password ingresados no coinciden", "error");</script>';
        $error = true;
    }
    ...
    if ($error === false) {
        ...
        if (mysqli_num_rows($resultado) > 0) {
                ...
                while ($valores = mysqli_fetch_array($resultado)) {
                    ...
                    $query = "UPDATE usuarios set clave = '$passwordNuevoHash' WHERE usuario = '$user'";
                    $result = mysqli_query($db, $query);

                    echo '<script>

                    swal("","Clave modificada con exito!","success")
                    .then((value) => {
        ...
        }else{ echo '<script>swal("Atención", "El usuario no existe", "error");</script>'; } 
    }   
}
?>